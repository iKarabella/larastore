...
class StoreWarehouseMarkRequest extends FormRequest
{
    /**
    ...
    public function rules(): array
    {
        return [
            'order_id'=>['numeric','required',Rule::exists('orders', 'id')->where(function (Builder $query) {
                $query->whereIn('status', [76,77,78,83,87,88,89]);
            })],
            'position_id'=>'numeric|required|exists:reserved_products,id',
            'warehouse_id'=>'numeric|required|exists:warehouses,id',
            'mark'=>'numeric|nullable|in:78,79',
        ];
    }
